...
// if we use signin/doctor signin/patient differently then we have to make two routes for login or dynamic url but why we create to route here kuyki dono me lag bhag same kaam horha signin me bs model me difference like jab patient sigin kre toh pateint model me dhundo and same for doctor toh isse accha signin me ek aur field add krdo role aur jo role hoga ke basis pr model search krna

// router.post("/signin/doctor", async (req, res) => {
//   const doctorDetails = req.body;
//   const response = SigninDetails.safeParse(doctorDetails);
...
//   }
//   try {
//     const user = await Doctor.findOne({ email: response.data.email });
//     if (!user) {
//       return res.status(404).json({ msg: "User not found." });
//     }
//     // Compare the password (use bcrypt's compare function)
...
//       // This is the payload.
//       // newPatient._id: This is the unique identifier (usually an auto-generated MongoDB ObjectId)
//       //user._id ---._id is in automated mongodb object and user is definded on line 101
//       // we are addingg role so jwt have role with the token and can be accessed for role based auth
...
//   const patientDetails = req.body;
//   const response = SigninDetails.safeParse(patientDetails);
...
//   }
//   try {
//     const user = await Patient.findOne({ email: response.data.email });
//     if (!user) {
//       return res.status(404).json({ msg: "User not found." });
//     }
//     // Compare the password (use bcrypt's compare function)
...
//now we can sign both doctor patient with this
